...
class UrbanSoundDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # samples = signal.data
        samples = self.transformation(signal.data)
        samples = self._resample_if_necessary(samples, sr)

        # mix signal down to mono-channel
    ...
    def _mix_down_if_necessary(samples):
...
mel_spectrogram = torchaudio.transforms.MelSpectrogram(
    sample_rate=SAMPLE_RATE, n_fft=FRAME_SIZE, hop_length=HOP_LENGTH, n_mels=NUM_MELS
)
